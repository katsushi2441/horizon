...
class HorizonOrchestrator:
    ...
    async def run(self, force_hours: int = None) -> None:
        ...
        try:
            ...
            important_items = self.filter_recent_summary_duplicates(important_items, today)
    ...
    def filter_recent_summary_duplicates(self, items: List[ContentItem], today: str) -> List[ContentItem]:
        """Drop URLs already used in earlier daily summaries."""
        seen_urls: set[str] = set()
        for path in sorted(self.storage.summaries_dir.glob("horizon-*-*.md"), reverse=True):
            if f"horizon-{today}-" in path.name:
                continue
            try:
        ...
        removed = 0
        for item in items:
            if self._normalize_url(str(item.url)) in seen_urls:
                removed += 1
                self.console.print(f"   dedup-history: drop {item.title[:70]}")
                continue
            filtered.append(item)

        if removed:
            self.console.print(f"🧹 Removed {removed} previously published items → {len(filtered)} fresh items\n")
        return filtered
    ...
    async def merge_topic_duplicates(self, items: List[ContentItem]) -> List[ContentItem]:
        ...
        def normalize_duplicate_group(group) -> list[int]:
            """Accept small JSON shape drift from local LLMs.

            Expected: [0, 3, 5]
            Common drift: [[0, 3, 5]] or [{"primary": 0, "duplicates": [3, 5]}]
            """
            if isinstance(group, dict):
                primary = group.get("primary")
                if primary is None:
                    primary = group.get("primary_index")
                duplicates = group.get("duplicates") or group.get("duplicate_indices") or []
                group = [primary] + (duplicates if isinstance(duplicates, list) else [duplicates])
            if isinstance(group, list) and len(group) == 1 and isinstance(group[0], list):
                group = group[0]
        ...
        for group in duplicate_groups:
            group = normalize_duplicate_group(group)
            if len(group) < 2:
                continue
            primary_idx = group[0]
    ...
    async def _enrich_important_items(self, items: List[ContentItem]) -> None:
        ...
        # enrichment_concurrency=0 の場合はスキップ（Ollama等の低速LLMで時間がかかりすぎるため）
        enrich_concurrency = getattr(self.config.ai, "enrichment_concurrency", 1)
        if enrich_concurrency == 0:
            self.console.print("📚 Enrichment skipped (enrichment_concurrency=0)")
            return
